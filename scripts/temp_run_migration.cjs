...
async function run() {
    ...
    try {
        // Try to create admin (ignoring error if exists)
        try {
           await pb.admins.create({
               email: 'admin@tutodecode.com',
               password: 'tutodecode123',
               passwordConfirm: 'tutodecode123'
           });
        ...
        // Auth
        await pb.admins.authWithPassword('admin@tutodecode.com', 'tutodecode123');
        console.log("Authenticated as admin.");
        
        const coursesData = courses; // from the concatenated file content
